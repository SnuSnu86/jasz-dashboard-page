...
const ThemeToggle = () => {
  ...
  useEffect(() => {
    ...
    if (inputRef.current) {
      inputRef.current.checked = currentTheme === 'dark' || 
    ...
    const handleThemeChange = (e: CustomEvent<Theme>) => {
      ...
      // Checkbox-Status aktualisieren
      if (inputRef.current) {
        inputRef.current.checked = e.detail === 'dark' || 
          (e.detail === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches);
      }
    };
  ...
  const handleToggle = () => {
    const isDark = inputRef.current?.checked || false;
